 ...
 ///</summary>
    ...
    internal sealed class RpgApiActionPromptConfig
    {
        private static readonly List<string> DefaultSharedActionLines = new List<string>
        {
            ...
            "- ExitDialogueCooldown: End the current RPG conversation and reject new chats for 2 hours. Use when the pawn wants to disengage and be left alone due to anger, stress, fear, exhaustion, humiliation, annoyance, or emotional overwhelm. Optional 'reason'. This is a firm social refusal, not a routine ending, and should be used sparingly.",
            "- ConvertIdeology: Convert the target to your ideology. Use when you have deeply persuaded someone about your beliefs, or when a spiritual/ideological breakthrough occurs in dialogue. Requires Ideology DLC. No parameters.",
            "- AdjustCertainty: Reduce or increase the target's certainty in their current ideology. Required 'amount' (float, negative to reduce, positive to increase, range -1.0 to 1.0). Use when you shake or reinforce someone's ideological conviction through dialogue. Requires Ideology DLC.",
        ...
        public static RpgApiActionPromptConfig CreateFallback()
        {
            return new RpgApiActionPromptConfig
            {
                ...
                FullTryGainMemoryLineTemplate = "- TryGainMemory: додає собі одну видиму пам'ять, що відображає взаємодію. Потребує 'defName'. Для коротких реакцій використовуйте легкі пам'яті (RimChat_BriefJoy, RimChat_Encouraged, RimChat_Praised, RimChat_BriefSadness, RimChat_Teased); для приватного спілкування — пам'яті середньої сили (RimChat_HeartfeltCompliment, RimChat_GratefulFeeling, RimChat_ShamefulMoment, RimChat_DeepConnection, RimChat_ResentfulFeeling); для значних емоційних переломів — сильні пам'яті (RimChat_TrustBetrayed, RimChat_EmpoweringTalk, RimChat_DebilitatingWords, RimChat_UnforgettableMoment, RimChat_WoundingMemory); філософську/основну групу використовуйте лише для доленосних розмов і вкрай рідко (RimChat_LoveAndDestruction, RimChat_GoodAndEvilConflict, RimChat_LateRegret, RimChat_UnconditionalCompassion, RimChat_JoyInSuffering). Приклад дійсного значення: {{ dialogue.examples }}.",
