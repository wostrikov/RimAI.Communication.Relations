...
{
        internal sealed class RPGPawnGroupChatPrompt : Dialog_RPGPawnGroupChatCollaborator
    {
        internal RPGPawnGroupChatPrompt(Dialog_RPGPawnGroupChat owner) : base(owner)
        {
        }


        internal List<ChatMessageData> BuildGroupRequestMessages(GroupChatParticipant speaker, bool isFirstTurn)
        {
            var request = new List<ChatMessageData>();

            // System prompt: speaker's persona + group context
            string systemPrompt = Owner.BuildGroupSystemPrompt(speaker, isFirstTurn);
            request.Add(new ChatMessageData { role = "system", content = systemPrompt });

            // Build context from accumulated turns
            string contextMessage = Owner.BuildTurnContextMessage(speaker, isFirstTurn);
            request.Add(new ChatMessageData { role = "user", content = contextMessage });

            return request;
        }

        internal string BuildGroupSystemPrompt(GroupChatParticipant speaker, bool isFirstTurn)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                basePrompt = Owner.BuildFallbackPersonaPrompt(speaker);
            }
        ...
        internal string BuildFallbackPersonaPrompt(GroupChatParticipant speaker)
        {
            var sb = new StringBuilder();
        ...
        internal string BuildTurnContextMessage(GroupChatParticipant speaker, bool isFirstTurn)
        {
            var sb = new StringBuilder();
