...
namespace Ustas.RimAI.Communication.Relations.UI
{
    ...
    ///</summary>
        internal sealed class RPGPawnDialoguePortraitDrag : Dialog_RPGPawnDialogueCollaborator
    {
        internal RPGPawnDialoguePortraitDrag(Dialog_RPGPawnDialogue owner) : base(owner)
        {
        }


        // ── drag state ──
        internal bool isDraggingInitiator;
        internal bool isSpringReturning;
        internal Vector2 dragCenter;       // current portrait center position
        internal Vector2 dragVelocity;     // spring velocity
        internal Vector2 restCenter;       // target rest position
        internal float dragScale = 1f;
        internal float dragScaleVel;

        // ── collision state ──
        internal bool collisionTriggeredThisDrag;
        internal float collisionShakeTime;
        internal float collisionFlashAlpha;
        internal const float CollisionShakeDuration = 0.35f;
        internal const float CollisionShakeMagnitude = 8f;
        internal const float CollisionFlashPeak = 0.6f;

        // ── physics constants ──
        internal const float SpringConstant = 10f;
        internal const float SpringDamping = 5f;
        internal const float DragFollowSpeed = 18f;
        internal const float ScaleSpeed = 8f;
        internal const float CollisionOverlapThreshold = 0.30f;

        // ── dynamic collision menu ──
        internal static readonly (string labelKey, string actionName, Func<Pawn, Pawn, bool> canShow)[] CollisionMenuActions =
        {
            ("RimChat_DragMenu_Romance",      "RomanceAttempt",      (t, i) => !Dialog_RPGPawnDialogue.HasLoveRelation(t, i)),
            ("RimChat_DragMenu_Marry",        "MarriageProposal",    (t, i) => !Dialog_RPGPawnDialogue.HasSpouseRelation(t, i)),
            ("RimChat_DragMenu_Breakup",      "Breakup",             (t, i) => Dialog_RPGPawnDialogue.HasLoveRelation(t, i)),
            ("RimChat_DragMenu_Divorce",      "Divorce",             (t, i) => Dialog_RPGPawnDialogue.HasSpouseRelation(t, i)),
            ...
            ("RimChat_DragMenu_ConvertIdeo",  "ConvertIdeology",     (t, i) => ModsConfig.IdeologyActive && t?.ideo != null),
            ("RimChat_DragMenu_AdjCertainty", "AdjustCertainty",     (t, i) => ModsConfig.IdeologyActive && t?.ideo != null),
        ...
        internal Rect GetInitiatorDragRect(Rect inRect)
        {
            ...
            return Owner.GetInitiatorPortraitRect(inRect);
        }

        internal void UpdatePortraitDrag(Rect inRect, float deltaTime)
        {
            Rect originalRect = Owner.GetInitiatorPortraitRect(inRect);
            ...
            // Collision detection
            Owner.UpdateCollisionDetection(inRect);

            // Collision animation
            Owner.UpdateCollisionAnimation(deltaTime);
        }

        internal void UpdateCollisionDetection(Rect inRect)
        {
            if (collisionTriggeredThisDrag || !isDraggingInitiator)
                return;

            Rect targetRect = Owner.GetTargetPortraitRect(inRect);
            Rect initiatorRect = Owner.GetInitiatorDragRect(inRect);

            if (!initiatorRect.Overlaps(targetRect))
                return;

            Rect overlap = Owner.RectIntersect(initiatorRect, targetRect);
            ...
            if (ratio >= CollisionOverlapThreshold)
            {
                ...
                Owner.ShowCollisionMenu();
            }
        }

        internal void UpdateCollisionAnimation(float deltaTime)
        {
        ...
        internal Rect RectIntersect(Rect a, Rect b)
        {
        ...
        internal float hoverSwingStartTime = -99f;
        internal bool hoverSwingWasOverLastFrame;
        internal const float HoverSwingDuration = 1.1f;
        internal const float HoverSwingAmplitudeDeg = 28f;
        internal const float HoverSwingFrequency = 12f;   // rad/s
        internal const float HoverSwingDecay = 3.0f;

        // ── drawing ──

        internal void DrawInitiatorPortraitWithDrag(Rect inRect)
        {
            Rect drawRect = Owner.GetInitiatorDragRect(inRect);
            Rect originalRect = Owner.GetInitiatorPortraitRect(inRect);
            ...
            if (isDraggingInitiator || isSpringReturning)
            {
                ...
                Owner.DrawPawnPortrait(new Rect(originalRect.x + 4f, originalRect.y + 4f, originalRect.width, originalRect.height), initiator, true);
            }
            ...
            if (swingActive)
            {
                ...
                Owner.DrawPawnPortrait(finalRect, initiator, true);
        ...
        internal bool TryStartInitiatorDrag(Rect inRect)
        {
            if (isDraggingInitiator || isSpringReturning)
                return false;

            Rect initiatorRect = Owner.GetInitiatorPortraitRect(inRect);
        ...
        internal void ShowCollisionMenu()
        {
            ...
            foreach (var (labelKey, actionName, canShow) in CollisionMenuActions)
            {
                ...
                {
                    Owner.ExecuteCollisionAction(capturedAction);
                }));
            }
        ...
        internal void ExecuteCollisionAction(string actionName)
        {
            ...
            bool success = Owner.ExecuteRpgAction(action);
            if (success && !string.Equals(actionName, "TryGainMemory", StringComparison.Ordinal))
            {
                Owner.NotifyActionSuccess(actionName, action);
            }
        }
        }
