...
namespace Ustas.RimAI.Communication.Relations.UI
{
 ...
 ///</summary>
        internal sealed class RPGPawnDialogueActions : Dialog_RPGPawnDialogueCollaborator
    {
        internal RPGPawnDialogueActions(Dialog_RPGPawnDialogue owner) : base(owner)
        {
        }

        internal void ApplyRPGAPIAndShowPopup(LLMRpgApiResponse apiRes)
        {
            if (apiRes == null)
            {
                return;
        ...
        internal void ExecuteRpgActions(List<LLMRpgApiResponse.ApiAction> actions)
        {
            ...
            {
                Owner.HandleDroppedResponse(validationReason);
                return;
            }
        ...
        internal bool ExecuteRpgAction(LLMRpgApiResponse.ApiAction action)
        {
            string normalizedName = Dialog_RPGPawnDialogue.NormalizeRpgActionName(action?.action);
            ...
            try
            {
                bool success = normalizedName switch
                {
                    ...
                    "TryGainMemory" => Owner.ExecuteTryGainMemory(action),
        ...
        internal bool TryValidateRpgActionExecutionContext(out string reason)
        {
        ...
        internal bool ExecuteExitDialogue(string reason, bool withCooldown)
        {
            if (withCooldown)
            {
                ...
                Owner.AddActionFeedback("RimChat_RPGActionToast_CooldownApplied".Translate(days.ToString("F1")), ActionInfoColor, 4.6f);
            }
        ...
        internal bool ExecuteRomanceAttempt(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteMarriageProposal(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteBreakup(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteDate(LLMRpgApiResponse.ApiAction action)
        {
            // "Date" has no dedicated hard state in vanilla; map it to guaranteed romantic state.
            return ExecuteRomanceAttempt(action);
        }

        internal bool ExecuteDivorce(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteTryAffectSocialGoodwill(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteReduceResistance(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteReduceWill(LLMRpgApiResponse.ApiAction action)
        {
            if (target?.guest == null || !target.IsPrisoner || action.amount <= 0)
            {
        ...
        internal bool ExecuteTryTakeOrderedJob(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteTriggerIncident(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteGrantInspiration(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteUnknownAction(string normalizedName, LLMRpgApiResponse.ApiAction action)
        {
            string rawAction = action?.action ?? normalizedName;
            Owner.AddActionFeedback("RimChat_RPGActionToast_Unknown".Translate(rawAction), ActionFailureColor);
            Owner.RecordSessionActionOutcome(rawAction, SessionActionOutcome.Failure, "RimChat_RPGActionToast_Unknown".Translate(rawAction).ToString());
            DebugLogger.LogInternal("RPGAction", $"Unknown RPG action ignored: {rawAction}");
            return false;
        }

        internal bool CanApplyRelationshipAction(string actionName)
        {
        ...
        internal PawnRelationDef ResolveRelationDef(string actionName, PawnRelationDef relationDef, string defName)
        {
        ...
        internal bool HasPairRelation(PawnRelationDef relationDef)
        {
        ...
        internal void AddPairRelation(PawnRelationDef relationDef)
        {
            if (relationDef == null || target?.relations == null || initiator == null)
            {
                return;
        ...
        internal void RemovePairRelation(PawnRelationDef relationDef)
        {
            if (relationDef == null || target?.relations == null || initiator == null)
            {
                return;
        ...
        internal void ClearOtherSpousesForMarriage(Pawn pawn, Pawn keepPartner, PawnRelationDef spouseDef, PawnRelationDef exSpouseDef)
        {
        ...
        internal List<InspirationDef> BuildInspirationCandidates(string defName)
        {
        ...
        internal bool TryStartInspiration(object handler, InspirationDef inspirationDef, string reason)
        {
        ...
        internal object[] BuildInspirationInvokeArgs(MethodInfo method, InspirationDef inspirationDef, string reason)
        {
        ...
        internal bool ExecuteConvertIdeology(LLMRpgApiResponse.ApiAction action)
        {
        ...
        internal bool ExecuteAdjustCertainty(LLMRpgApiResponse.ApiAction action)
        {
            if (!ModsConfig.IdeologyActive)
            {
        ...
        internal void HandleNpcExitDialogue(string reason)
        {
        ...
        internal void NotifyActionSuccess(string actionName, LLMRpgApiResponse.ApiAction action)
        {
            string actionLabel = GetRpgActionLabel(actionName);
            Owner.AddActionFeedback("RimChat_RPGActionToast_Success".Translate(actionLabel), ActionSuccessColor);
            string reason = action?.reason;
            string detail = ResolveActionDetailForHistory(actionName, action);
            Owner.RecordSessionActionOutcome(actionName, SessionActionOutcome.Success, reason, detail);
            LogRpgActionDebug($"RPG action success: {actionName}");
        }

        internal string ResolveActionDetailForHistory(string actionName, LLMRpgApiResponse.ApiAction action)
        {
            ...
            ThoughtDef def = Owner.ResolveTryGainMemoryThoughtDef(action?.defName ?? string.Empty, out _);
            if (def == null)
            {
        ...
        internal void NotifyActionFailure(string actionName, string reason)
        {
            string actionLabel = GetRpgActionLabel(actionName);
            Owner.AddActionFeedback("RimChat_RPGActionToast_Failure".Translate(actionLabel, reason), ActionFailureColor);
            Owner.RecordSessionActionOutcome(actionName, SessionActionOutcome.Failure, reason);
            LogRpgActionDebug($"RPG action failed: {actionName}, reason={reason}");
        }

        internal void NotifyActionError(string actionName, string reason)
        {
            string actionLabel = GetRpgActionLabel(actionName);
            Owner.AddActionFeedback("RimChat_RPGActionToast_Error".Translate(actionLabel), ActionErrorColor);
            Owner.RecordSessionActionOutcome(actionName, SessionActionOutcome.Error, reason);
            DebugLogger.WarningGated($"RPG action error: {actionName}");
        }

        internal string GetRpgActionLabel(string actionName)
        {
