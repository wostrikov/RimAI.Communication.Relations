...
{
        internal sealed class RPGPawnGroupChatHistory : Dialog_RPGPawnGroupChatCollaborator
    {
        internal RPGPawnGroupChatHistory(Dialog_RPGPawnGroupChat owner) : base(owner)
        {
        }


        internal bool isHistoryPanelOpen;
        internal Vector2 historyScrollPos = Vector2.zero;
        internal const float HistoryPanelMinW = 600f;
        internal const float HistoryPanelMaxW = 860f;
        internal const float HistoryPanelMinH = 200f;
        internal const float HistoryPanelMaxH = 560f;
        ...
        internal void DrawHistoryPanel(Rect inRect)
        {
            ...
            for (int i = 0; i < dialogPages.Count; i++)
            {
                ...
                totalH += Dialog_RPGPawnGroupChat.MeasureGroupHistoryEntryHeight(entry, viewWidth) + 2f;
            }
            Text.Font = prevFont;
            ...
            {
                string entry = dialogPages[i].speakerName + ": " + dialogPages[i].text;
                float entryH = Dialog_RPGPawnGroupChat.MeasureGroupHistoryEntryHeight(entry, viewWidth);
        ...
        internal bool TryHandleHistoryPanelClick()
        {
        ...
        internal static float MeasureGroupHistoryEntryHeight(string text, float width)
        {
