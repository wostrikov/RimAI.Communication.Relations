...
{
    /// <summary>Prisoner ransom prepare/commit APIs.</summary>
    internal sealed class GameAIPrisonerRansom : GameAIInterfaceCollaborator
    {
        internal GameAIPrisonerRansom(GameAIInterface owner) : base(owner)
        {
        }


        internal readonly PrisonerRansomService prisonerRansomService = new PrisonerRansomService();
        internal const string RansomPaymentModeSilver = "silver";
        internal const float BatchRansomReleaseTimeoutMultiplier = 1.5f;
        ...
        {
            if (!TryValidatePrepareContext(faction, parameters, out RelationsSettings settings, out APIResult validationFailure))
            {
                return validationFailure;
            }

            if (!GameAIAirdropDrop.TryReadIntParameter(parameters, "target_pawn_load_id", out int pawnLoadId) || pawnLoadId <= 0)
            {
            ...
            if (!GameAIAirdropDrop.TryReadIntParameter(parameters, "offer_silver", out int offeredSilver) || offeredSilver <= 0)
            {
                return FailFastRansom(
                    "invalid_offer_silver",
                    "RimChat_RansomInvalidOfferSystem".Translate().ToString(),
                    $"invalid offer_silver={GameAIAirdropDrop.ReadString(parameters, "offer_silver")}");
            }

            string paymentModeRaw = GameAIAirdropDrop.ReadString(parameters, "payment_mode");
            ...
            bool hasBatchTargetCount = GameAIAirdropDrop.TryReadIntParameter(parameters, "batch_target_count", out int batchTargetCount);
            ...
            var preparedData = new PrisonerRansomPrepareData
            {
                ...
                BatchGroupId = GameAIAirdropDrop.ReadString(parameters, "batch_group_id"),
        ...
        internal RansomContractRecord BuildContract(
        ...
        internal static int ResolveContractTimeoutTicks(PrisonerRansomPrepareData preparedData, RelationsSettings settings)
        {
        ...
        internal static string NormalizeRansomPaymentMode(string paymentMode, RelationsSettings settings)
        {
        ...
        internal static APIResult FailFastRansom(string code, string playerMessage, string debugDetail)
        {
        ...
        internal static bool TryValidatePrepareContext(
            Faction faction,
            Dictionary<string, object> parameters,
            out RelationsSettings settings,
            out APIResult failure)
        {
            settings = RelationsMod.Instance?.InstanceSettings;
            failure = null;
            if (settings == null)
            {
        ...
        internal static int ResolveSoftenedRansomPenalty(int goodwillPenalty, bool triggerRaid, string reasonTag)
        {
        ...
        internal static int ApplyRansomRelationPenalty(Faction faction, int goodwillPenalty, bool forceHostile)
        {
            if (faction == null)
            {
                return 0;
        ...
        internal static void EnsureHostileRelation(Faction faction)
        {
        ...
        internal static bool TryDeliverRansomSilver(Map map, int silverAmount, RelationsSettings settings, out IntVec3 dropCell)
        {
            ...
            if (!GameAIAirdropDrop.TryFindAirdropCell(map, out dropCell))
            {
                return false;
            }

            int maxStacks = Mathf.Clamp(settings?.ItemAirdropMaxStacksPerDrop ?? 8, 1, 100);
            List<Thing> silverStacks = GameAIAirdropDrop.BuildStacks(ThingDefOf.Silver, silverAmount, maxStacks);
