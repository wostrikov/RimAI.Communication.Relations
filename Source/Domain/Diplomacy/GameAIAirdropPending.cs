...
{
    /// <summary>Forced and timeout pending airdrop selection.</summary>
    internal sealed class GameAIAirdropPending : GameAIInterfaceCollaborator
    {
        internal GameAIAirdropPending(GameAIInterface owner) : base(owner)
        {
        }


        internal APIResult TryBuildForcedSelection(
            ...
            out int maxByBudget)
        {
            ...
            {
                return Owner.Parts.AirdropDrop.BuildSelectionFailure("selection_out_of_candidates", "selected_def is not available in current candidate list.");
            }
            ...
            GameAIAirdropDrop.ComputeLegalCountWindow(budget, selectedRecord, candidatePack, settings, out maxByBudget, out int maxBySystem, out hardMax);
            ...
            {
                int baseCount = GameAIAirdropDrop.ResolveFamilyDefaultCount(intent?.Family ?? ItemAirdropNeedFamily.Unknown);
        ...
        internal APIResult BuildTimeoutPendingSelection(
            ItemAirdropIntent intent,
            ItemAirdropCandidatePack candidatePack,
            int budget,
            RelationsSettings settings,
            ...
            bool allowEmptyOptions = false)
        {
            ...
            if (candidatePack?.Candidates == null || candidatePack.Candidates.Count == 0)
            {
                return Owner.Parts.AirdropDrop.BuildSelectionFailure(resolvedFailureCode, resolvedFailureReason);
            }
            ...
            foreach (ItemAirdropCandidate candidate in candidatePack.Candidates.Take(5))
            {
                ...
                GameAIAirdropDrop.ComputeLegalCountWindow(budget, candidate.Record, candidatePack, settings, out _, out _, out int hardMax);
        ...
        internal static string BuildPendingSelectionAuditDetails(ItemAirdropPendingSelectionData pendingData)
        {
