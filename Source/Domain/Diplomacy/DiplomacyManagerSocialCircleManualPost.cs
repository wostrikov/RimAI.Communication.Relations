...
namespace Ustas.RimAI.Communication.Relations.DiplomacySystem
{
    ...
    /// </summary>
        internal sealed class DiplomacyManagerSocialCircleManualPost : GameComponent_DiplomacyManagerCollaborator
    {
        internal DiplomacyManagerSocialCircleManualPost(GameComponent_DiplomacyManager owner) : base(owner)
        {
        }
        ...
        public ManualSocialPostResult TryPublishManualPlayerSocialPost(string title, string body)
        {
            ...
            PublicSocialPost post = Owner.CreateManualPlayerSocialPost(normalizedTitle, normalizedBody, currentTick);
            if (post == null)
            {
            ...
            List<Faction> targetFactions = Owner.SelectManualReactionFactions(normalizedTitle, normalizedBody);
            Owner.TriggerManualPostResponses(post, targetFactions);
        ...
        internal PublicSocialPost CreateManualPlayerSocialPost(string title, string body, int currentTick)
        {
        ...
        internal List<Faction> SelectManualReactionFactions(string title, string body)
        {
            ...
            List<FactionScore> scored = candidates
                .Select(faction => new FactionScore(faction, Owner.ScoreManualReactionFaction(faction, content, category, sentiment)))
        ...
        internal void TriggerManualPostResponses(PublicSocialPost post, List<Faction> targetFactions)
        {
            ...
            {
                pushManager.RegisterCustomTrigger(Owner.BuildManualPostTriggerContext(faction, post));
            }
        }

        internal NpcDialogueTriggerContext BuildManualPostTriggerContext(Faction faction, PublicSocialPost post)
        {
            string body = post?.Content?.Trim() ?? string.Empty;
            string reason =
                $"manual_social_post|title={GameComponent_DiplomacyManager.SanitizeManualReasonSegment(post?.Headline)}|body={GameComponent_DiplomacyManager.SanitizeManualReasonSegment(body)}";
        ...
        internal bool IsEligibleManualReactionFaction(Faction faction)
        {
            return faction != null &&
                   !faction.IsPlayer &&
                   !faction.defeated &&
        ...
        internal float ScoreManualReactionFaction(Faction faction, string content, SocialPostCategory category, int sentiment)
        {
            if (faction == null)
            {
            ...
            float score = 0f;
            score += GameComponent_DiplomacyManager.CountMentionHits(normalizedContent, faction.Name) * 5f;
            score += GameComponent_DiplomacyManager.CountMentionHits(normalizedContent, faction.def?.label) * 3f;
            score += GameComponent_DiplomacyManager.CountMentionHits(normalizedContent, faction.def?.defName) * 2f;
        ...
        internal static int CountMentionHits(string content, string token)
        {
        ...
        internal static string SanitizeManualReasonSegment(string text)
        {
        ...
        internal sealed class FactionScore
        {
