 ...
 ///</summary>
    public static class SocialCircleActionResolver
    {
        ...
        public static void ResolveAndExecute(GameComponent_DiplomacyManager manager, SocialCircleState state, int currentTick)
        {
            ...
            if (!(Ustas.RimAI.Communication.Relations.Module.RelationsMod.Instance?.InstanceSettings?.EnableSocialCircleAutoActions ?? false)) return;
            ...
            for (int i = 0; i < state.ActionIntents.Count; i++)
            {
                ...
                ActionResult result = RelationsInteractionAdapter.Execute(action, intent.Faction);
                if (result.IsSuccess)
                {
                    intent.Score = 0f;
