...
using PendingGenerationContext = Ustas.RimAI.Communication.Relations.PawnRpgPush.GameComponent_PawnRpgDialoguePushManager.PendingGenerationContext;
...
namespace Ustas.RimAI.Communication.Relations.PawnRpgPush
{
 ...
 ///</summary>
        internal sealed class PawnRpgDialoguePushManagerGeneration : GameComponent_PawnRpgDialoguePushManagerCollaborator
    {
        internal PawnRpgDialoguePushManagerGeneration(GameComponent_PawnRpgDialoguePushManager owner) : base(owner)
        {
        }


        internal void StartGeneration(PawnRpgTriggerContext context, Pawn npcPawn, Pawn playerPawn)
        {
            if (context == null || npcPawn == null || playerPawn == null || !Owner.IsValidTargetFaction(context.Faction))
            {
                return;
            }
            ...
            AIChatServiceAsync.Instance.StartCoroutine(Owner.BuildAndSendRoutine(context, npcPawn, playerPawn));
        }

        internal IEnumerator BuildAndSendRoutine(PawnRpgTriggerContext context, Pawn npcPawn, Pawn playerPawn)
        {
            ...
            if (!Owner.IsValidTargetFaction(context.Faction) || !AIChatServiceAsync.Instance.IsConfigured())
                yield break;
            ...
            Owner.AppendRecentRpgContext(messages, npcPawn, playerPawn);
            ...
            string reason = Owner.BuildReasonText(context);
            string userPrompt =
        ...
        internal void OnGenerationSuccess(string requestId, string response)
        {
            ...
            Owner.DeliverMessage(pending.Context, pending.NpcPawn, pending.PlayerPawn, message);
        }
        ...
        internal void DeliverMessage(PawnRpgTriggerContext context, Pawn npcPawn, Pawn playerPawn, string text)
        {
            if (context == null || npcPawn == null || playerPawn == null || !Owner.IsValidTargetFaction(context.Faction))
            {
                return;
            ...
            int currentTick = Find.TickManager?.TicksGame ?? 0;
            Owner.CleanupExpiredMessageHashes(currentTick);
            ...
            string contentHash = GameComponent_PawnRpgDialoguePushManager.ComputeContentHash(text);
            ...
            TaggedString title = Owner.GetLetterTitle(context, npcPawn, playerPawn);
            LetterDef letterDef = Owner.GetLetterDef(context);
            ...
            Owner.RecordRpgDelivery(currentTick);
            PawnRpgNpcPushState npcState = Owner.GetOrCreateNpcState(npcPawn);
            ...
            else if (!Owner.CanBypassGlobalCooldown(context))
            {
                lastColonyDeliveredTick = currentTick;
            }
        }

        internal TaggedString GetLetterTitle(PawnRpgTriggerContext context, Pawn npcPawn, Pawn playerPawn)
        {
        ...
        internal LetterDef GetLetterDef(PawnRpgTriggerContext context)
        {
        ...
        internal List<ChatMessageData> BuildGenerationMessages(PawnRpgTriggerContext context, Pawn npcPawn, Pawn playerPawn)
        {
            var messages = new List<ChatMessageData>();
        ...
        internal void AppendRecentRpgContext(List<ChatMessageData> messages, Pawn npcPawn, Pawn playerPawn)
        {
            ...
            string npcMood = Owner.TryGetMoodPercent(npcPawn, out float mood) ? $"{(mood * 100f):F0}%" : "unknown";
            int opinion = Owner.GetOpinion(npcPawn, playerPawn);
            bool intimate = Owner.HasIntimateRelation(npcPawn, playerPawn);
        ...
        internal string BuildReasonText(PawnRpgTriggerContext context)
        {
            if (context == null)
            {
                return "unknown";
