...
namespace Ustas.RimAI.Communication.Relations.Memory
{
    ...
    /// </summary>
        internal sealed class RpgNpcDialogueArchiveManagerWarmup : RpgNpcDialogueArchiveManagerCollaborator
    {
        internal RpgNpcDialogueArchiveManagerWarmup(RpgNpcDialogueArchiveManager owner) : base(owner)
        {
        }
        ...
        public void BeginPromptMemoryWarmup(Pawn targetNpc, Pawn currentInterlocutor = null)
        {
            ...
            lock (_syncRoot)
            {
                ...
                try
                {
                    ...
                    sourceDir = Owner.ResolveArchiveSourceDirectory();
                }
        ...
        internal void WarmupCacheInBackground(string saveKey, string sourceDir, int targetPawnLoadId)
        {
            ...
            lock (_syncRoot)
            {
                ...
                foreach (KeyValuePair<int, RpgNpcDialogueArchive> pair in loaded)
                {
                    ...
                    {
                        RpgNpcDialogueArchiveManager.MergeArchiveData(existing, pair.Value);
                    }
                    else
                    {
        ...
        internal static Dictionary<int, RpgNpcDialogueArchive> LoadArchiveSnapshot(string sourceDir, string saveKey)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                try
                {
                    ...
                    if (!RpgNpcDialogueArchiveManager.IsArchiveOwnedBySaveKey(archive, saveKey))
                    {
                        continue;
                    }
                    ...
                    {
                        RpgNpcDialogueArchiveManager.MergeArchiveSnapshot(existing, archive);
                    }
                    else
                    {
        ...
        internal static bool IsArchiveOwnedBySaveKey(RpgNpcDialogueArchive archive, string saveKey)
        {
            if (archive == null)
            {
                return false;
        ...
        internal static void MergeArchiveSnapshot(RpgNpcDialogueArchive target, RpgNpcDialogueArchive incoming)
        {
        ...
        internal void FlushPendingWarmupCompressionLockless(int tick)
        {
            ...
            for (int i = 0; i < pendingTargets.Count; i++)
            {
                ...
                {
                    Owner.TryScheduleSessionCompression(archive, safeTick);
                }
            }
        }
        }
