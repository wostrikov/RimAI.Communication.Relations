...
namespace Ustas.RimAI.Communication.Relations.Config
{
    ...
    /// </summary>
        internal static class ApiUsabilityDiagnosticServiceHelpers
    {

        internal static ApiUsabilityDiagnosticResult BuildSuccess(
            ...
            string successDetail = "")
        {
            return new ApiUsabilityDiagnosticResult
            {
                ...
                TechDetail = ApiUsabilityDiagnosticService.BuildTechDetail(step, endpoint, httpCode, successDetail),
                HttpCode = httpCode,
                EndpointUsed = endpoint ?? string.Empty,
                ElapsedMs = ApiUsabilityDiagnosticService.GetElapsedMilliseconds(startedAtUtc),
                Steps = steps.ToList(),
                PlayerHintKeys = new List<string>(),
                DebugRequestText = requestPayload ?? string.Empty,
                DebugResponseText = ApiUsabilityDiagnosticService.TruncateDebugPayload(responsePayload),
                ModelName = modelName ?? string.Empty,
                IsCloud = isCloud
            };
        }

        internal static ApiUsabilityDiagnosticResult BuildFailureFromProbe(
            ApiUsabilityStep step,
        ...
        {
            ApiUsabilityErrorCode code = forceCode ?? ApiUsabilityDiagnosticService.ClassifyErrorCode(probe.HttpCode, probe.Error, isCloud);
        ...
        internal static ApiUsabilityDiagnosticResult BuildFailure(
            ApiUsabilityStep step,
            ...
            string responsePayload)
        {
            var failedStep = new ApiUsabilityStepResult
            {
                ...
                Success = false,
                ErrorCode = code,
                TechDetail = ApiUsabilityDiagnosticService.BuildTechDetail(step, endpoint, httpCode, details),
                HttpCode = httpCode,
                EndpointUsed = endpoint ?? string.Empty,
                ElapsedMs = ApiUsabilityDiagnosticService.GetElapsedMilliseconds(startedAtUtc)
            };
            ...
            return new ApiUsabilityDiagnosticResult
            {
                ...
                Steps = allSteps,
                PlayerHintKeys = ApiUsabilityDiagnosticService.GetHintKeys(code),
                DebugRequestText = requestPayload ?? string.Empty,
                DebugResponseText = ApiUsabilityDiagnosticService.TruncateDebugPayload(responsePayload),
                ModelName = modelName ?? string.Empty,
                IsCloud = isCloud
        ...
        internal static ApiUsabilityStepResult BuildStepSuccess(ApiUsabilityStep step, string endpoint, DateTime startedAtUtc)
        {
        ...
        internal static void NotifyProgress(Action<ApiUsabilityProgress> onProgress, ApiUsabilityStep step, int current, int total)
        {
        ...
        internal static ApiUsabilityErrorCode ClassifyErrorCode(long httpCode, string error, bool isCloud)
        {
        ...
        internal static List<string> GetHintKeys(ApiUsabilityErrorCode code)
        {
            return code switch
            {
        ...
        internal static string BuildMissingModelDetail(string targetModel, List<string> discoveredModels)
        {
        ...
        internal static string BuildTechDetail(ApiUsabilityStep step, string endpoint, long httpCode, string details)
        {
        ...
        internal static long GetElapsedMilliseconds(DateTime startedAtUtc)
        {
            return Math.Max(0L, (long)(DateTime.UtcNow - startedAtUtc).TotalMilliseconds);
        }

        internal static string BuildModelTagPrefix(string model)
        {
        ...
        internal static List<string> ExtractQuotedValues(string json, string fieldToken)
        {
            ...
            while ((index = json.IndexOf(fieldToken, index, StringComparison.OrdinalIgnoreCase)) >= 0)
            {
                ...
                int closeQuote = ApiUsabilityDiagnosticService.FindClosingQuote(json, openQuote + 1);
                if (closeQuote < 0)
                {
                    break;
                }
        ...
        internal static int FindClosingQuote(string text, int startIndex)
        {
            bool escaped = false;
        ...
        internal static string TruncateDebugPayload(string payload)
        {
        ...
        internal struct ApiUsabilityProbeRequest
        {
            public string Url;
        ...
        internal struct ApiUsabilityProbeResponse
        {
        ...
        internal struct ApiUsabilityCloudRuntime
        {
        ...
        internal struct ApiUsabilityLocalServiceProbe
        {
        ...
        internal enum ApiUsabilityLocalServiceType
        {
            Unknown = 0,
            OpenAiCompatible = 1,
            Ollama = 2
